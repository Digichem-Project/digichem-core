...
class Result_widget(Row_widget):
    ...
    def load_inner(self):
        ...
        return urwid.Columns([
            urwid.Text(self.row_item._value.metadata.identity_string),
            # TODO: Careful, what if result has no file?
            urwid.Text(self.row_item._value.metadata.log_files[0])
        ])
...
class Result_list(Row_browser):
    ...
    def __init__(self, top, start_dir = None, rearrangeable = True, initial_results = None):
        """
    ...
    def add_to_list(self):
        ...
        # Parse (in parallel).
        results = parse_multiple_calculations(*self.selector.selected, alignment_class = alignment, init_func = self.program.subprocess_init, processes = self.selector.num_CPUs)
