...
class Row_widget(urwid.WidgetPlaceholder):
    ...
    def __init__(self, row_item, movable = False):
        """
    ...
    def index(self):
        ...
        """
        return self.row_item.row_list.index(self.row_item)
        
    def get_widget(self, reload):
        """
    ...
    def get_controls(self, reload):
        """
        Get the buttons we use for control.
        """
        if self._controls is not None or reload:
    ...
    def move(self, position, amount):
        ...
        self.row_item.row_list.body.insert(new_index, self.row_item.row_list.body.pop(cur_index))
        
        # Update.
        self.row_item.row_list.update_positions()
        
        # Set ourself as the focus.
        self.row_item.row_list.body.set_focus(new_index if new_index < len(self.row_item.row_list.body) else len(self.row_item.row_list.body) -1)
...
class Row_walker(urwid.SimpleFocusListWalker):
    ...
    def __init__(self, data, row_list):
        """
        Constructor for Row_walker objects.
        
        :param data: List of data (Row_items) to display.
        :param row_list: The parent row_list we belong to.
        """
        self.row_list
        super().__init__(data)
...
class Row_list(urwid.ListBox):
    ...
    def __init__(self, data, rearrangeable = True):
        """
        
        :param rearrangeable: Whether the rows of this Row_list can be rearranged.
        """
        super().__init__(Row_walker(data))