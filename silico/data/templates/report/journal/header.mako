...
<div class="header">
	...
	<div class="header__titleArea">
 		<div class="header__title">A report for the calculation of ${report.result.metadata.human_calculations_string} of ${report.result.metadata.molecule_name}</div>
 		...
 		% if len(report.result.results) == 0:
 		## Only one calc.
 		The calculation was performed using the ${report.result.metadata.package} software package at the ${report.result.metadata.level_of_theory} level of theory.
 		% else:
 		## Multiple calcs.
 		The calculations were performed using the ${report.result.metadata.package} software package(s) at the ${report.result.metadata.level_of_theory} level of theory.
 		% endif
...
## 			% if len(energy) > 0:
## 			The total ${energy.human_energy_type} (${energy.energy_type}) energy of the system was found to be ${"{:.2f}".format(energy.final)} eV after ${len(energy} steps.
## 			% endif
## 		% endfor
## 		##
## 		## Orbitals.
...
## 		The most intense vibrational frequencies were calculated at ${report.images['simulated_IR_graph'].selected_peaks(0, 5)} cm<sup>-1</sup>,
...
## 			${len(report.result.excited_states)} total excited states were calculated with ${report.result.excited_states.multiplicity_strings} multiplicity.
## 			% if 'simulated_absorption_graph' in report.images:
## 				The most intense absorption peaks were calculated at ${report.images['simulated_absorption_graph'].selected_peaks(0, 5)} nm.
## 			% endif
## 			## S(1) and T(1).
...
## 			% elif T1 is not None:
## 				The lowest energy tripley excited state (S<sub>1</sub>) was calculated to be ${"{:.2f}".format(T1.energy)} eV (${"{:.0f}".format(T1.wavelength)} nm).
## 			% endif
## 		% endif
## 		##
## 		## Emission.
