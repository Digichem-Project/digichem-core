...
def digichem_options(tmpdir_factory):
    config = digilib.config.get_config()
    
    # Change the default DB locations to prevent SPAM.
    config['databases'] = [
        "{{name: test1, path: {}}}".format(
            Path(tmpdir_factory.mktemp("digichem_database1"), "tmp.db")
        ),
        "{{name: test2, path: {}}}".format(
            Path(tmpdir_factory.mktemp("digichem_database2"), "tmp.db")
        ),
        "{{name: test3, auto_fill: False, path: {}}}".format(
            Path(tmpdir_factory.mktemp("digichem_database3"), "tmp.db")
        ),
    ]
