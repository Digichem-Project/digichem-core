...
from digilib.parse.base import File_parser_abc
from digilib.result.tdm import Transition_dipole_moment
from digilib.result.result import Result_set
from digilib.result.metadata import Metadata
from digilib.result.orbital import Molecular_orbital_list
from digilib.result.atom import Atom_list
from digilib.result.excited_state import Excited_state_list
from digilib.result.energy import Energies
from digilib.result.ground_state import Ground_state
from digilib.result.soc import SOC_list
from digilib.result.dipole_moment import Dipole_moment
from digilib.result.vibration import Vibrations_list
from digilib.result.emission import Relaxed_excited_state
import digilib.log
from digilib.result.alignment.base import Alignment, Minimal
from digilib.result.nmr import NMR_list
...
class Yaml_multi_parser(Dump_multi_parser_abc):
    ...
    def _parse(self):
        ...
        """
        # Read that file.
        digilib.log.get_logger().info("Parsing calculation result '{}'".format(self.description))
...
class Dump_parser_abc(File_parser_abc):
    ...
    def process(self):
        ...
        try:
            ...
            digilib.log.get_logger().warning("Failed to parse emission data", exc_info = True)
