...
class Configurable_list(OrderedDict):
    ...
    def __init__(self, *args, **kwargs):
        """
        Constructor for Configurable_list objects.
        """
        super().__init__(*args, **kwargs)
        self.aliases = {}
    
    @classmethod
    def from_list(self, configurables):
        """
        Construct a Configurable_list object from a list of configurables.
        """
        conf_list = self()
        conf_list.update_from_list(configurables)
        return conf_list

    def update_from_list(self, configurables):
        """
        Add a list of configurables to this Configurable list object.
        """
        for configurable in configurables:
            # First, if this configurable has UPDATE_CATEGORY set, update all categories that match.
            if configurable.UPDATE_CATEGORY is not None:                
                # Get all configurables that match.
                configurables_to_update = self.get_from_category(configurable.UPDATE_CATEGORY)
                
                # Panic if no match.
                if len(configurables_to_update) == 0:
                    raise Configurable_exception(configurable, "Cannot UPDATE_CATEGORY; their are no configurables with the category '{}'".format(configurable.UPDATE_CATEGORY))
                
                # Delete UPDATE_CATEGORY before we merge, it will be meaningless afterwards.
                del(configurable.UPDATE_CATEGORY)
                
                # Merge each.
                for configurable_to_update in configurables_to_update:
                    deepmerge.always_merger.merge(configurable_to_update._configurable_options, configurable._configurable_options)
            
            else:
                # No UPDATE_CATEOGRY, just a 'normal' configurable.
                # Check if we've already got this config.
                if configurable.NAME in self:
                    # Merge.
                    deepmerge.always_merger.merge(self[configurable.NAME]._configurable_options, configurable._configurable_options)
                
                else:
                    # Add.
                    self[configurable.NAME] = configurable
                    
                # Add the configurable's ALIASES to our list.
                for alias in self[configurable.NAME].ALIASES:
                    self.aliases[alias] = self[configurable.NAME]

    def get_from_category(self, category):
        """
        Get a number of configurables from this list that have a given category set as one of their categories.
        
        :param category: The category to search for.
        :returns: A list of configurables that have category set as one of their CATEGORY.
        """
        found_configurables = []
        
        for configurable in self.values():
            if category in configurable.CATEGORY:
                found_configurables.append(configurable)
                
        return found_configurables

    def get_config(self, identifier):
        """
        Get a Configurable object that matches a given ID.
         
        :raises Configurable_class_exception: If the Configurable cannot be found .
        :param identifier: Either the name or index of the config to get.
        """
        if isinstance(identifier, int) or identifier.isdigit():
            ...
            try:
                ...
                try:
                    ...
                    # TODO: Would be better if we printed the actual type of configurable here (eg, method, calculation etc). 
                    raise Silico_exception("could not find Configurable with NAME/ALIAS '{}'".format(identifier)) from None


    def validate(self):
        """
        Validate all of the Configurable objects contained within this list.
        """
        for configurable in self.values():
