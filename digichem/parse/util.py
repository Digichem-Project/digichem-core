...
def parse_calculation(*log_files, options, ornt = None, ornt_args = (), parse_all = False, format_hint = "auto", keep_archive = False, **auxiliary_files):
    """
    ...
    try:
        ...
        if parse_all:
            results = from_log_files(*open_log_files, format_hint = format_hint, **auxiliary_files).process_all(options, ornt = ornt, ornt_args = ornt_args)
        
        else:       
            results = from_log_files(*open_log_files, format_hint = format_hint, **auxiliary_files).process(options, ornt = ornt, ornt_args = ornt_args)
...
def multi_parser(log_files, auxiliary_files, *, options, ornt = None, ornt_args = (), format_hint = "auto", keep_archive = False):
        """
        ...
        try:    
            return parse_calculation(*logs, options = options, ornt = ornt, ornt_args = ornt_args, parse_all = True, format_hint = format_hint, keep_archive = keep_archive, **auxiliary_files)
            
        except Exception:
            digichem.log.get_logger().warning("Unable to parse calculation result file '{}'; skipping".format(logs[0]), exc_info = True)
            return None

def parse_multiple_calculations(*log_files, auxiliary_files = None, options, ornt = None, ornt_args = (), pool = None, init_func = None, init_args = None, format_hint = "auto", processes = 1, keep_archive = False):
    """
    ...
    try:
        result_lists = list(
            filterfalse(lambda x: x is None,
                pool.starmap(partial(multi_parser, options = options, ornt = ornt, ornt_args = ornt_args, format_hint = format_hint, keep_archive = keep_archive), zip_longest(log_files, auxiliary_files, fillvalue = {}))
...
def parse_and_merge_calculations(*log_files, auxiliary_files = None, options, ornt = None, ornt_args = (), format_hint = "auto", inner_pool = None, keep_archive = False):
    """
    ...
    """    
    parsed_results = parse_multiple_calculations(*log_files, options = options, ornt = ornt, ornt_args = ornt_args, format_hint = format_hint, pool = inner_pool, auxiliary_files = auxiliary_files, keep_archive = keep_archive)
...
def multi_merger_parser(log_files, auxiliary_files, *, options, ornt = None, ornt_args = (), format_hint = "auto" , inner_pool = None, keep_archive = False):
        """
        The inner function which will be called in parallel to parse files.
        """
        try:
            return parse_and_merge_calculations(*log_files, options = options, ornt = ornt, ornt_args = ornt_args, format_hint = format_hint, inner_pool = inner_pool, auxiliary_files = auxiliary_files, keep_archive = keep_archive)
...
def parse_and_merge_multiple_calculations(*multiple_results, options, ornt = None, ornt_args = (), format_hint = "auto", init_func = None, init_args = None, processes = None, auxiliary_files = None, keep_archive = False):
    """
    ...
    try:
        ...
        result_lists = list(
            filterfalse(lambda x: x is None,
                map(partial(multi_merger_parser, options = options, ornt = ornt, ornt_args = ornt_args, format_hint = format_hint, inner_pool = pool, keep_archive = keep_archive), multiple_results, auxiliary_files)
            )
        )
