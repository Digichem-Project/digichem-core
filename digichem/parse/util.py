...
def parse_calculation(*log_files, options, ornt = None, ornt_args = (), parse_all = False, format_hint = "auto", keep_archive = False, parser_options = {}, **auxiliary_files):
    ...
    try:
        ...
        if parse_all:
            results = from_log_files(*open_log_files, format_hint = format_hint, parser_options = parser_options, **open_aux_files).process_all(options, ornt = ornt, ornt_args = ornt_args)
        
        else:       
            results = from_log_files(*open_log_files, format_hint = format_hint, parser_options = parser_options, **open_aux_files).process(options, ornt = ornt, ornt_args = ornt_args)
